...
def create_sequences(df, sequence_length, stride=1):
    ...
    for fault in fault_types:
        ...
        # Use stride here:
        for i in range(0, len(df_fault) - sequence_length + 1, stride):
...
def save_sequences(input_directory, output_directory, sequence_length, stride=1):
    """
    ...
    - stride: Step size to slide the window.
    """
    ...
    for file_name in tqdm(os.listdir(input_directory), desc="Generating sequences"):
        if file_name.endswith('.csv'):
            ...
            sequences, labels = create_sequences(df, sequence_length, stride)
...
def add_speed_feature_and_save(input_directory, output_directory, sequence_length, stride=1):
    if not os.path.exists(output_directory):
        os.makedirs(output_directory)
