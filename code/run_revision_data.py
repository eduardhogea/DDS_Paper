...
from revision_data_processing import overview_csv_files, process_pgb_data
    ...
    if args.train_model:
        ...
        for file in sorted(os.listdir(sequences_directory)):
            if "_train_scaled_sequences.npy" in file:
                ...
                X.append(np.concatenate((X_train, X_test), axis=0))
                y.append(np.concatenate((y_train, y_test), axis=0))

        # Convert X and y lists to arrays after the loop is complete
        ...
        for fold, (train_idx, val_idx) in enumerate(kf.split(X, y)):
            ...
            def sat_phi1(features):
                ...
                phi1 = Forall(x, Implies(p([x, class_0]), Not(p([x, class_0]))), p=5)
                return phi1.tensor

            @tf.function
            def sat_phi2(features):
                x = ltn.Variable("x", features)
                phi2 = Forall(x, Implies(p([x, class_0]), Not(p([x, class_1]))), p=5)
                return phi2.tensor
        ...
        concatenate_and_delete_ltn_csv_files(results_path_ltn, "results_kaggle/results_ltn.csv")
