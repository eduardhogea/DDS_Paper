...
def main():
    ...
    # initialize device and console
    get_device()
    console = Console()
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    # 1) thresholds from 0.45 to 1.00 step 0.05
    thresholds = [round(0.45 + 0.05 * i, 2) for i in range(int((1.0 - 0.45) / 0.05) + 1)]
    best_acc_dict = {}

    # 2) prepare base output directories
    os.makedirs(results_path_ltn, exist_ok=True)
    base_plots_dir   = os.path.join(results_path_ltn, "plots_cluj")
    base_metrics_dir = os.path.join(results_path_ltn, "metrics_cluj")
    os.makedirs(base_plots_dir, exist_ok=True)
    os.makedirs(base_metrics_dir, exist_ok=True)

    # 3) loop over each merge_cosine_threshold
    for tau in thresholds:
        global merge_cosine_threshold
        merge_cosine_threshold = tau
        console.print(f"\n\n===== Running experiment with merge_cosine_threshold = {tau} =====\n")

        # per‐tau output dirs
        plots_dir   = os.path.join(base_plots_dir,   f"tau_{tau}")
        metrics_dir = os.path.join(base_metrics_dir, f"tau_{tau}")
        ...
        centroids  = recompute_centroids(model, emb_loader, device)  # uses current tau
        sim_preds  = build_similarity_predicates(model, centroids)

        # training setup
        best_acc  = 0.0
        best_path = os.path.join(model_save_directory, f"best_model_tau_{tau}.pt")
        epochs_list, train_sat_list, val_sat_list, val_acc_list = [], [], [], []

        # 4) training loop
        for epoch in trange(1, epochs + 1, desc=f"Training (tau={tau})"):
            model.train()
            running = 0.0
            ...
            # recompute centroids & similarity predicates with updated model
            centroids = recompute_centroids(model, emb_loader, device)
            sim_preds = build_similarity_predicates(model, centroids)

        # 5) save per‐epoch metrics & plots
        ...
        # store best accuracy for this tau
        best_acc_dict[tau] = best_acc
        console.print(f"Completed tau={tau}. Best val_acc={best_acc:.4f}\n")

    # 7) summary table of deltas from baseline tau=0.55
    baseline = best_acc_dict.get(0.55, None)
    if baseline is None:
        console.print("[red]Error:[/] No results for baseline tau=0.55 found.")
    else:
        console.print("\nThreshold | Best Acc | Δ from 0.55")
        console.print("----------|----------|-----------")
        for tau in thresholds:
            delta = best_acc_dict[tau] - baseline
            console.print(f"{tau:>8.2f} | {best_acc_dict[tau]:>8.4f} | {delta:+9.4f}")
